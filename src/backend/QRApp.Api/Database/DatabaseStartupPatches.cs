...
public static class DatabaseStartupPatches
{
    public static async Task ApplyDatabaseStartupPatchesAsync(this WebApplication app)
    {
        ...
        await using var orderItemCancellationRequestCommand = new NpgsqlCommand(OrderItemCancellationRequestSql, connection);
        await orderItemCancellationRequestCommand.ExecuteNonQueryAsync();
...
""";
    ...
    private const string OrderItemCancellationRequestSql = """
...
LANGUAGE sql STABLE AS $$
    SELECT oi."OrderItemId",oi."OrderId",oi."MenuItemId",oi."MenuItemVariantId",oi."MenuItemName",oi."VariantName",oi."ItemNote",oi."DietTypeCode",oi."UnitPrice",(oi."Quantity" - oi."CancelledQuantity")::integer,round(oi."UnitPrice" * (oi."Quantity" - oi."CancelledQuantity"),2),req."OrderItemCancellationRequestId",req."RequestedQuantity",req."Reason",req."RequestedAtUtc"
    FROM "OrderItems" oi
...
LANGUAGE sql STABLE AS $$
    SELECT oi."OrderItemId",oi."OrderId",oi."MenuItemId",oi."MenuItemVariantId",oi."MenuItemName",oi."VariantName",oi."ItemNote",oi."DietTypeCode",oi."UnitPrice",oi."Quantity",oi."LineTotal",oi."CancelledQuantity",(oi."Quantity" - oi."CancelledQuantity")::integer,round(oi."UnitPrice" * (oi."Quantity" - oi."CancelledQuantity"),2),oi."CancelledReason",oi."CancelledAtUtc",req."OrderItemCancellationRequestId",req."RequestedQuantity",req."Reason",req."RequestedAtUtc"
    FROM "OrderItems" oi
    JOIN "Orders" o ON o."OrderId"=oi."OrderId"
    LEFT JOIN "OrderItemCancellationRequests" req ON req."OrderItemId"=oi."OrderItemId" AND req."StatusCode"='Pending'
    WHERE oi."TenantId"=p_tenantid AND oi."BranchId"=p_branchid
...
RETURNS TABLE("OrderId" uuid,"TenantId" uuid,"BranchId" uuid,"TableId" uuid,"OrderStatusCode" varchar,"CustomerName" varchar,"CustomerWhatsApp" varchar,"Notes" varchar,"SubtotalAmount" numeric,"TotalAmount" numeric,"AppliedBranchOfferId" uuid,"AppliedOfferTitle" varchar,"AppliedOfferDiscountAmount" numeric,"CreatedAtUtc" timestamptz,"UpdatedAtUtc" timestamptz)
LANGUAGE plpgsql AS $$
DECLARE ctx record; item_row record; pending_quantity integer := 0;
BEGIN
    SELECT o."OrderId",o."TenantId",o."BranchId",o."TableId",o."CustomerId",o."OrderStatusCode" INTO ctx
    FROM "Orders" o
    ...
    IF ctx."CustomerId" IS NULL OR NOT EXISTS (SELECT 1 FROM "CustomerDeviceAccessTokens" access WHERE access."TenantId"=ctx."TenantId" AND access."BranchId"=ctx."BranchId" AND access."CustomerId"=ctx."CustomerId" AND access."TokenHash"=p_tokenhash AND access."RevokedAtUtc" IS NULL AND access."ExpiresAtUtc" > public.app_now()) THEN PERFORM public.raise_app_error(51716); END IF;
    IF ctx."OrderStatusCode" NOT IN ('Placed','Accepted','Preparing') THEN PERFORM public.raise_app_error(51717); END IF;
    IF EXISTS (SELECT 1 FROM "OrderBills" ob WHERE ob."TenantId"=ctx."TenantId" AND ob."BranchId"=ctx."BranchId" AND ob."OrderId"=p_orderid AND ob."PaymentStatusCode" <> 'Unpaid') THEN PERFORM public.raise_app_error(51714); END IF;
    IF COALESCE(p_cancelquantity,0) <= 0 THEN PERFORM public.raise_app_error(51711); END IF;
    IF NULLIF(btrim(p_reason),'') IS NULL OR length(btrim(p_reason)) > 300 THEN PERFORM public.raise_app_error(51712); END IF;
    SELECT oi.* INTO item_row FROM "OrderItems" oi WHERE oi."TenantId"=ctx."TenantId" AND oi."BranchId"=ctx."BranchId" AND oi."OrderId"=p_orderid AND oi."OrderItemId"=p_orderitemid FOR UPDATE;
    IF item_row."OrderItemId" IS NULL THEN PERFORM public.raise_app_error(51713); END IF;
