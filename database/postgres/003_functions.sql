...
LANGUAGE sql AS $$ SELECT NULL::uuid,NULL::uuid,NULL::uuid,NULL::uuid,NULL::varchar,NULL::varchar,NULL::varchar,NULL::numeric,NULL::varchar,NULL::boolean,NULL::boolean,NULL::integer,NULL::timestamptz,NULL::timestamptz,NULL::varchar,NULL::varchar,NULL::text WHERE false; $$;
...
RETURNS TABLE("MenuItemId" uuid,"TenantId" uuid,"BranchId" uuid,"MenuCategoryId" uuid,"CategoryName" varchar,"Name" varchar,"Description" varchar,"Price" numeric,"DietTypeCode" varchar,"IsAvailable" boolean,"IsActive" boolean,"DisplayOrder" integer,"CreatedAtUtc" timestamptz,"UpdatedAtUtc" timestamptz,"ImageUrl" varchar,"ImageAltText" varchar,"VariantsJson" text)
LANGUAGE sql STABLE AS $$
    SELECT mi."MenuItemId",mi."TenantId",mi."BranchId",mi."MenuCategoryId",mc."Name",mi."Name",mi."Description",mi."Price",mi."DietTypeCode",mi."IsAvailable",mi."IsActive",mi."DisplayOrder",mi."CreatedAtUtc",mi."UpdatedAtUtc",mi."ImageUrl",mi."ImageAltText",
...
CREATE OR REPLACE FUNCTION public.menuitem_create(p_menuitemid uuid,p_tenantid uuid,p_branchid uuid,p_menucategoryid uuid,p_name text,p_description text,p_price numeric,p_diettypecode text,p_isavailable boolean,p_displayorder integer,p_imageurl text,p_imagealttext text,p_variantsjson text)
RETURNS TABLE("MenuItemId" uuid,"TenantId" uuid,"BranchId" uuid,"MenuCategoryId" uuid,"CategoryName" varchar,"Name" varchar,"Description" varchar,"Price" numeric,"DietTypeCode" varchar,"IsAvailable" boolean,"IsActive" boolean,"DisplayOrder" integer,"CreatedAtUtc" timestamptz,"UpdatedAtUtc" timestamptz,"ImageUrl" varchar,"ImageAltText" varchar,"VariantsJson" text)
LANGUAGE plpgsql AS $$
BEGIN
...
LANGUAGE sql STABLE AS $$
    SELECT b."BranchId",b."Name",b."LogoUrl",mc."MenuCategoryId",mc."Name",mc."DisplayOrder",mi."MenuItemId",mi."Name",mi."Description",mi."Price",mi."DietTypeCode",mi."DisplayOrder",mi."ImageUrl",mi."ImageAltText",
...
CREATE OR REPLACE FUNCTION public.orderfeedback_createfromqrtoken(p_qrtoken text,p_orderid uuid,p_orderfeedbackid uuid,p_rating integer,p_comment text) RETURNS TABLE("OrderFeedbackId" uuid,"TenantId" uuid,"BranchId" uuid,"OrderId" uuid,"CustomerId" uuid,"Rating" integer,"Comment" varchar,"CreatedAtUtc" timestamptz) LANGUAGE plpgsql AS $$ DECLARE order_context record; BEGIN SELECT o."TenantId",o."BranchId",o."OrderId",o."CustomerId",o."CustomerName",o."CustomerWhatsApp" INTO order_context FROM "Orders" o JOIN "BranchTables" bt ON bt."TableId"=o."TableId" WHERE o."OrderId"=p_orderid AND bt."QrToken"=p_qrtoken; IF NOT FOUND THEN PERFORM public.raise_app_error(51709); END IF; RETURN QUERY WITH saved AS (INSERT INTO "OrderFeedback" ("OrderFeedbackId","TenantId","BranchId","OrderId","Rating","Comment","CustomerName","CustomerWhatsApp") VALUES (p_orderfeedbackid,order_context."TenantId",order_context."BranchId",p_orderid,p_rating,p_comment,order_context."CustomerName",order_context."CustomerWhatsApp") ON CONFLICT ("OrderId") DO UPDATE SET "Rating"=EXCLUDED."Rating","Comment"=EXCLUDED."Comment","CustomerName"=EXCLUDED."CustomerName","CustomerWhatsApp"=EXCLUDED."CustomerWhatsApp" RETURNING *) SELECT saved."OrderFeedbackId",saved."TenantId",saved."BranchId",saved."OrderId",order_context."CustomerId",saved."Rating",saved."Comment",saved."CreatedAtUtc" FROM saved; END; $$;
CREATE OR REPLACE FUNCTION public.orderfeedback_getbyqrtoken(p_qrtoken text,p_orderid uuid) RETURNS TABLE("OrderFeedbackId" uuid,"TenantId" uuid,"BranchId" uuid,"OrderId" uuid,"CustomerId" uuid,"Rating" integer,"Comment" varchar,"CreatedAtUtc" timestamptz) LANGUAGE sql STABLE AS $$ SELECT f."OrderFeedbackId",f."TenantId",f."BranchId",f."OrderId",o."CustomerId",f."Rating",f."Comment",f."CreatedAtUtc" FROM "OrderFeedback" f JOIN "Orders" o ON o."OrderId"=f."OrderId" JOIN "BranchTables" bt ON bt."TableId"=o."TableId" WHERE f."OrderId"=p_orderid AND bt."QrToken"=p_qrtoken; $$;
...
CREATE OR REPLACE FUNCTION public.report_orderitemsbyorder(p_tenantid uuid,p_orderid uuid) RETURNS TABLE("OrderItemId" uuid,"MenuItemId" uuid,"MenuItemVariantId" uuid,"MenuItemName" varchar,"VariantName" varchar,"ItemNote" varchar,"DietTypeCode" varchar,"UnitPrice" numeric,"Quantity" integer,"LineTotal" numeric) LANGUAGE sql STABLE AS $$ SELECT oi."OrderItemId",oi."MenuItemId",oi."MenuItemVariantId",oi."MenuItemName",oi."VariantName",oi."ItemNote",oi."DietTypeCode",oi."UnitPrice",oi."Quantity",oi."LineTotal" FROM "OrderItems" oi WHERE oi."TenantId"=p_tenantid AND oi."OrderId"=p_orderid ORDER BY oi."RowId"; $$;
