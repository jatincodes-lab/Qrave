...
BEGIN
    ...
    UPDATE "OrderItems"
    SET "CancelledQuantity"="CancelledQuantity" + p_cancelquantity,
    ...
    SET "SubtotalAmount"=active_subtotal,
        ...
        "OrderStatusCode"=CASE WHEN active_subtotal <= 0 THEN 'Cancelled' ELSE "OrderStatusCode" END,
    ...
    IF bill_row."OrderBillId" IS NOT NULL THEN
        ...
        SET "SubtotalAmount"=active_subtotal,
            ...
            "TotalAmount"=payable_total,
            "UpdatedAtUtc"=public.app_now()
        WHERE "OrderBillId"=bill_row."OrderBillId";
    END IF;
...
LANGUAGE sql STABLE AS $$
    ...
    JOIN (
        ...
        LIMIT 5
    ) recent ON recent."OrderId"=oi."OrderId"
...
LANGUAGE sql STABLE AS $$
    SELECT oi."MenuItemName",
           oi."VariantName",
           SUM(oi."Quantity" - oi."CancelledQuantity")::integer,
           COUNT(DISTINCT oi."OrderId")::integer,
           SUM(round(oi."UnitPrice" * (oi."Quantity" - oi."CancelledQuantity"),2))
    FROM "OrderItems" oi
    JOIN "Orders" o ON o."OrderId"=oi."OrderId"
